...
#ifndef REVISION
#define REVISION Revision: unknown
#endif
char run_file[1000];
  ...
  if (argc == 2) {
    ...
    *strstr(outdir, ".par") = '\0';
    ...
    if (processor0) {
      ...
      if (1) printf("Safety first: checking data before overwriting previous directory.\n");
    ...
    if (processor0) {
      ...
      time(&t);
      char svn_file[1000];
      sprintf(svn_file,"%s/svn_info",outdir);
      fp = fopen(svn_file, "w");
      if (!fp) errorexits("failed opening %s", svn_file);
      fprintf(fp,"Start  %s  T = %2.6f\n  %s\n",
	      ctime(&t),0.,STRINGIZE_VALUE_OF(REVISION));
      fclose(fp);
      
      /* running file */
      sprintf(run_file,"%s/running",outdir);
      fp = fopen(run_file, "w");
      if (!fp) errorexits("failed opening %s", run_file);
      fprintf(fp,"just started\n");
      fclose(fp);
    }
    bampi_barrier();
...
void advance_levelandsublevels(tG *g, int l)
{
  ...
  if (pr) printf("entering alas %d\n", l);
  
  /* touch the run_file in order to see if the job is not yet finished */
  if (processor0) {
    FILE *fp = fopen(run_file, "w");
    if (!fp) errorexits("failed opening %s", run_file);
    time_t t;
    time(&t);
    fprintf(fp,"yes, still running   ... last update at %s",ctime(&t));
    fclose(fp);
  }
  
  /* do a openmp test output to see how many threads are supported by the system */
#ifdef OPENMP
  bampi_openmp_test();
#endif
  ...
  if (1) prmemory_adv("before move_box",g);
  if (move_box(g, l+1)) {
  ...
  if (1) prmemory_adv("after move_box",g);
  ...
  /* info */
  if (l == 0) prmemory("after advance");
  if (pr) printf("leaving alas %d\n", l);
}
...
void advance_level(tG *g, int l) 
{
  ...
  /* evolve */
  if (0) printf("l%d evolve\n", l);
  RunFun(EVOLVE, level); 

  /* set boundaries */
  if (0) printf("l%d setrefbound\n", l);
  if (0 && l) prvar(level, "g");
  //setrefbound(level);
  if (0 && l) prvar(level, "g");
...
{
  if (processor0)  system_rmrf(run_file);
  prdivider(0);
  free_grid(g);
  bampi_barrier();
}
