...
void AppPar(const char *name, const char *value)
{
    ...
    char *v;
    if (findparameter(name,0)==0) makeparameter(name, value, "");
    else {
...
void EvalBracket(tdb* first, tdb* last)
{
  ...
  while (1) {
    ...
    if (ptr->o==1 && f!=ptr) {
      if (f->b==ptr->b) {
        l = ptr;
        if (f->next==l) errorexit("something is wrong with the brackets");
  ...
  for (i=8; i>1; i--) {
    ...
    while (first!=last) {
      ...
      if        (i==8 && ptr==first && ptr->o==2) {
        if (!ptr->next || ptr->next->o!=0) errorexit("wrong syntax8");
        ptr->v = +ptr->next->v;
        ptr->o = 0;
        del = 1;
      } else if (i==8 && ptr->prev && ptr->prev->o==6 && ptr->o==2) {
        if (!ptr->next || ptr->next->o!=0) errorexit("wrong syntax8.2");
        ptr->v = +ptr->next->v;
        ptr->o = 0;
        del = 1;
      } else if (i==7 && ptr==first && ptr->o==3) {
        if (!ptr->next || ptr->next->o!=0) errorexit("wrong syntax7");
        ptr->v = -ptr->next->v;
        ptr->o = 0;
        del = 1;
      } else if (i==7 && ptr->prev && ptr->prev->o==6 && ptr->o==3) {
        if (!ptr->next || ptr->next->o!=0) errorexit("wrong syntax7.2");
        ptr->v = -ptr->next->v;
        ptr->o = 0;
        del = 1;
      } else if (i==6 && ptr->next && ptr->next->o==6) {
        if (!ptr->next->next || ptr->o!=0 || ptr->next->next->o!=0) errorexit("wrong syntax6");
        ptr->v = pow(ptr->v,ptr->next->next->v);
        del = 2;
      } else if (i==5 && ptr->next && ptr->next->o==5) {
        if (!ptr->next->next || ptr->o!=0 || ptr->next->next->o!=0) errorexit("wrong syntax5");
        ptr->v = ptr->v/ptr->next->next->v;
        del = 2;
      } else if (i==4 && ptr->next && ptr->next->o==4) {
        if (!ptr->next->next || ptr->o!=0 || ptr->next->next->o!=0) errorexit("wrong syntax4");
        ptr->v = ptr->v*ptr->next->next->v;
        del = 2;
      } else if (i==3 && ptr->next && ptr->next->o==3) {
        if (!ptr->next->next || ptr->o!=0 || ptr->next->next->o!=0) errorexit("wrong syntax3");
        ptr->v = ptr->v-ptr->next->next->v;
        del = 2;
      } else if (i==2 && ptr->next && ptr->next->o==2) {
        if (!ptr->next->next || ptr->o!=0 || ptr->next->next->o!=0) errorexit("wrong syntax2");
        ptr->v = ptr->v+ptr->next->next->v;
        del = 2;
      } else {
        del = 0;
      ...
      if (del==2) {
        ...
        if (ptr->next->next->next) {
          ...
          if (ptr->next->next==last) last=ptr;
          free(ptr->next->next);
          free(ptr->next);
          ptr->next = tmp;
          ptr->next->prev = ptr;
...
int iterate_parameters(void)
{
  ...
  if (!Getv("iterate_parameters", "yes")) {
    ...
    /* return 1 for first call, but 0 for second call, which exits bam */
    if (ncall == 1) 
      return 0;
    ncall = 1;
    return 1;
  ...
  *strstr(newoutdir, ".par") = '\0';
  ...
  if (newvalue) {
    ...
    if (processor0) {
      system_rmrf(outdirp);
      system_move(newoutdir, outdirp); 
      system_mkdir(newoutdir);
    }
    bampi_barrier();
    free(outdirp);
    ncall++;
