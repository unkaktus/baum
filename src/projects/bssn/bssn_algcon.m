...
tocompute = {
  ...
  detg == matrixdet[g],
  Cinstruction == "if (detg<=0.01) { PROBLEM; /*errorexit(\" detg <= 0.\");*/ } ",
  detginv == 1/detg,
  ...
  detg == matrixdet[g],
  (*Cinstruction == "if (detg<=0.) { PROBLEM; /*errorexit(\" detg <= 0.\");*/ } ",*)
  detginv == 1/detg,
...
endfunctionloop = "} endfor_ijk_openmp; /*  if (stop) errorexit(\"\"); */ /* loop ijk */";
...
variabledeclarations[] := Module[{},
  ...
  pr["const int store          = Getv(\"bssn_register_algcon\", \"store\");\n"];
];    
