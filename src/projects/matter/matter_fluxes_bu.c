...
void matter_fluxes(tVarList *unew, tVarList *upre, double c, tVarList *ucur, tVarList *primVars)
{
  bampi_openmp_start
  ...
  forallboxes(level) {
    ...
    // x-direction
    bampi_openmp_loop_collapse2
    for (k=0; k<=kmax; k++) {
      for (j=0; j<=jmax; j++) {

	compute_flux1d(1,box, j,k, unew,upre,c,ucur,primVars, fluxs1d);
	...
	compute_flux1d(2,box, i,k, unew,upre,c,ucur,primVars, fluxs1d);
	...
	compute_flux1d(3,box, i,j, unew,upre,c,ucur,primVars, fluxs1d);
		    ...
		    double **fluxs1d)
{
  ...
  if(addlinear)   camr_fact = dtime*ooh; /* for euler integration*/
  else            camr_fact = ooh;       /* for all forms of RK  */
    for (v=0; v<MATTER.NVf; v++){
...
#if (1)
  if (f_ijk_notfinite) for (v=0; v<MATTER.NVf; v++) fluxs1d[v][l]=0.; // set all to 0 (default)
#endif
  ...
  for (l=1; l<=N; l++) {
    ...
    // ijk index 
    if (direction==1)       ijk = box->noffset + (l-1)+ box->m*dir1 + box->m*box->n*dir2;
    else if (direction==2)  ijk = box->noffset + dir1 + box->m*(l-1)+ box->m*box->n*dir2;
    else                    ijk = box->noffset + dir1 + box->m*dir2 + box->m*box->n*(l-1);
    
    
    // Handle ATM pt 
    if ( (MATTER.USEMASK) && (mask[ijk]>0.99) ) {
      // add zero to RHS				
      for (v=0; v<MATTER.NVf; v++) r[v][ijk] -= 0.;
      // go next point
      continue;
    }
...
#if (1)
    if (f_ijk_notfinite) for (v=0; v<MATTER.NVf; v++) fluxs1d[v][l]=0.; 
    // set all to 0 (default)                                                           
#endif

    
    // Compute div F 
    for (v=0; v<MATTER.NVf; v++) deltaF[v] = ooh* (fluxs1d[v][l] - fluxs1d[v][l-1]);
	...
	for (v=0; v<MATTER.NVf; v++) printf(" %d \t %e %e %e\n",v,fluxs1d[v][l],fluxs1d[v][l-1],deltaF[v]);
      }
    ...
    // Add to RHS
    if (addlinear) for (v=0; v<MATTER.NVf; v++) r[v][ijk] -= c*deltaF[v];
    else           for (v=0; v<MATTER.NVf; v++) r[v][ijk] -= deltaF[v];

    //save fluxes for the c_amr_XXX in type B-cells
     if ((MATTER.USECAMR)) { 

         if (mask_B[ijk]) { 
             sdir = directioncamr(mask_B[ijk],direction);  
               if(sdir == 1) {
                for (v=0; v<MATTER.NVf; v++) 
      ...
      // and in type A-cells
         if (mask_A[ijk]) { 
             sdir = directioncamr(mask_A[ijk],direction);
               if(sdir == 1) {
                for (v=0; v<MATTER.NVf; v++) 
                    camr[v][ijk] -= camr_fact*fluxs1d[v][l-1];
               } 
...
void extrapolate(double *f, int l,int lb, int rb)
{
  ...
  if (l<lb) f[l] = f[lb];// + (double)(l-lb)*( f[lb+1]-f[lb] );
  if (l>rb) f[l] = f[rb];// + (double)(l-rb)*( f[rb]-f[rb-1] );
}
...
void extrapolate2(double *f, int l,int lb, int rb, const int ord)
{
  ...
  if (l<lb) f[l] = f[lb] + ord*((double)(l-lb)*( f[lb+1]-f[lb] ));
  if (l>rb) f[l] = f[rb] + ord*((double)(l-rb)*( f[rb]-f[rb-1] ));
}
		    ...
		    double **fluxs1d)
{
  ...
  int Nmax    = 0;
  int NVg     = 8;
  int Nfields = NVg + MATTER.NVw + MATTER.NVq + 1 ;
  int index_w = NVg;
  ...
  for(l=0; l<NVg       ; l++) g1d[l] = buffer[          l] + off;
  for(m=0; m<MATTER.NVw; m++) w1d[m] = buffer[index_w + m] + off;
  for(n=0; n<MATTER.NVq; n++) q1d[n] = buffer[index_q + n] + off;
  
  /* mask information */
  double *mask1d = buffer[Nfields-1] + off;

  // copy physical data form 3d to 1d arrays ... [1..N]
  for (l=0; l<N; l++) {
    if (direction==1)       {ijk = box->noffset + l    + box->m*dir1 + box->m*box->n*dir2; 
    //if (l == 0) printf(" %d \n", box->noffset);
     }
    else if (direction==2)  ijk = box->noffset + dir1 + box->m*l    + box->m*box->n*dir2;
    else                    ijk = box->noffset + dir1 + box->m*dir2 + box->m*box->n*l;
    
    // mask
    if (MATTER.USEMASK) mask1d[l+1]  = level->v[MATTER.INDX_VAR_mask][ijk];
  ...
  for (l=-Nghost+1; l<=N+Nghost; l++) { 
    if (l==1) l=N+1;
    for (m=0; m<Nfields; m++) extrapolate(buffer[m]+off,   l,1,N);
  }
  
  
  // finally call the flx 1d routine
  MATTER.hrsc_flx1d( g1d, w1d, q1d, mask1d, N,Nghost, direction, fluxs1d);
