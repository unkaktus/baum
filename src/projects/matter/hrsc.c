...
int hrsc_startup(tL *level)
{
  ...
  if (Getv("hrsc_flux", "HLL"))
  {
    ...
    if (Getv("physics", "grmhd") || Getv("physics", "grmhdY"))
      MATTER.hrsc_riemsol1d = hrsc_numflx1d_hll_grmhd;
  }
  ...
  else if (Getv("hrsc_rec", "PPM6"))
  {
    ...
    errorexit("unkown reconstruction");
  ...
  if (Getv("hrsc_use_lower", "yes"))
  {
    ...
    else if (Getv("hrsc_rec", "PPM6"))
    {
      ...
      errorexit("unkown reconstruction for lower order scheme");
  }
  ...
  // check no ghosts
  if (MATTER.HRSC_NGHOST < 2)
    errorexit(" you need at least 2 hrsc_nghosts");
  if (Geti("bampi_nghosts") < 4)
    errorexit(" you need at least 4 bampi_nghosts");
  ...
  {
    if (MATTER.HRSC_NGHOST < 3)
      errorexit(" you need at least 3 hrsc_nghosts for high-order rec");
    if (Geti("bampi_nghosts") < 6)
      errorexit(" you need at least 4 bampi_nghosts for high-order rec ... use 6!");
  }
}
