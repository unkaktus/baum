...
int matter_init(tL *level)
{
  ...
  if (firstcall)
  {
    ...
    if (MATTER.USECAMR)
      vlpushvl(evolve_vars, camrVars);
    if (MATTER.USENLS)
      vlpushvl(evolve_vars, chiVarsf);
    ...
    if (MATTER.USENLS)
    {
      for (MATTER.INDX_VAR_chi = 0; MATTER.INDX_VAR_chi < evolve_vars->n; MATTER.INDX_VAR_chi++)
        if (Ind("nls_chi_nue") == evolve_vars->index[MATTER.INDX_VAR_chi])
          break;
    }
    ...
    if (MATTER.USECAMR)
    {
      if (MATTER.INDX_VAR_camr == evolve_vars->n)
        errorexit(" index of camr not found inside evolved variables \n");
    }
    if (MATTER.USENLS)
    {
      if (MATTER.INDX_VAR_chi == evolve_vars->n)
  ...
  {
    if (level->l > level->grid->lmin)
      MATTER.chi_prolong(chiVarsc, chiVarsf, 1);
...
void matter_rhs_prepare(tVarList *unew, tVarList *upre, double c, tVarList *ucur)
{
  ...
  // set ptrs to rhs
  bampi_openmp_parallel_for_collapse2 for (int v = 0; v < MATTER.NVq; v++)
  {
  ...
  if (MATTER.USENLS)
  {
    bampi_openmp_parallel_for_collapse2 for (int v = 0; v < MATTER.NVchi; v++)
    {
      for (int i = 0; i < nnodes; i++)
      {
...
void matter_rhs(tVarList *unew, tVarList *upre, double c, tVarList *ucur)
{
  ...
  if (MATTER.USENLS)
  {
    ...
    {
      if (PR)
        printf("# ===> matter NLS chi\n");
      MATTER.chi_compute(unew, upre, c, ucur);
    }
  }

  if (PR)
    printf("# ===> matter rhs sources\n");
