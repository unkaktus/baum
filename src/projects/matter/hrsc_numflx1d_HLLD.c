              ...
              double RBxR, double RByR, double RBzR)
{
  ...
  if (fabs(lambda_aR - lambda_aL) < 1e-5 * fabs(lambdaR - lambdaL))
    return lambda_aR - lambda_aL;
  else
  {
    HLLD_c_B(lambda_aL,
             vxaL, vyaL, vzaL,
                     ...
                     double delta_lambda)
{
  ...
  for (count = 0; count <= countmax; count++)
  {
    ...
    if (DEBUG)
      printf("a, b, fb, iter = %+.6e %+.6e %+.6e %i \n", a, b, fb, count);

    error = fabs(fb);
    if (error <= errormax || error != error)
      break;
  ...
  *res = b;
  ...
  if (error < 10 * errormax)
    return 1;
  // else                return 0;
  else
  {
    if (PR)
      printf("HLLD pressure root finder failed, switched to HLL \n");
    if (PR)
      printf("a, b, fb, iter = %+.6e %+.6e %+.6e %i \n", a, b, fb, count);
    return 0;
  }
                          ...
                          int nf)
{
  ...
  if (amins >= v_interface)
    for (int v = 0; v < nf - 1; v++)
    {
      fHLL[v] = fL[v];
      qHLL[v] = qL[v];
    }
  else if (aplus <= v_interface)
    for (int v = 0; v < nf - 1; v++)
    {
      fHLL[v] = fR[v];
      qHLL[v] = qR[v];
                          ...
                          int nf)
{
  ...
  if (DEBUG)
    printf("fD, fTau, fSx, fSy, fSz, fBx, fBy, fBz = %+.6e, %+.6e, %+.6e, %+.6e, %+.6e, %+.6e, %+.6e, %+.6e \n", fHLL[0], fHLL[1], fHLL[2], fHLL[3], fHLL[4], fHLL[5], fHLL[6], fHLL[7]);
  if (DEBUG)
    printf(" D,  Tau,  Sx,  Sy,  Sz,  Bx,  By,  Bz = %+.6e, %+.6e, %+.6e, %+.6e, %+.6e, %+.6e, %+.6e, %+.6e \n", qHLL[0], qHLL[1], qHLL[2], qHLL[3], qHLL[4], qHLL[5], qHLL[6], qHLL[7]);
                 ...
                 double *f, double *q, int nf)
{
  ...
  if (v_interface <= lambdaL)
    for (int v = 0; v < nf - 1; v++)
    {
      f[v] = fL[v];
      q[v] = qL[v];
    }
  else if (v_interface >= lambdaR)
    for (int v = 0; v < nf - 1; v++)
    {
      f[v] = fR[v];
      q[v] = qR[v];
  ...
  else
  {
    ...
    double RDL = lambdaL * qL[0] - fL[0];
    double RDR = lambdaR * qR[0] - fR[0];
    double REL = lambdaL * qL[1] - fL[1];
    double RER = lambdaR * qR[1] - fR[1];
    double RSxL = lambdaL * qL[2] - fL[2];
    double RSxR = lambdaR * qR[2] - fR[2];
    double RSyL = lambdaL * qL[3] - fL[3];
    double RSyR = lambdaR * qR[3] - fR[3];
    double RSzL = lambdaL * qL[4] - fL[4];
    double RSzR = lambdaR * qR[4] - fR[4];
    double RBxL = lambdaL * qL[5] - fL[5];
    double RBxR = lambdaR * qR[5] - fR[5];
    double RByL = lambdaL * qL[6] - fL[6];
    double RByR = lambdaR * qR[6] - fR[6];
    double RBzL = lambdaL * qL[7] - fL[7];
    double RBzR = lambdaR * qR[7] - fR[7];
    ...
    if (convergence & (hL >= pc) & (hR >= pc))
    {
      ...
      if (fabs(lambda_aR - lambda_aL) < 1e-5 * fabs(lambdaR - lambdaL))
      {
        lambdac = 0.5 * (lambda_aR + lambda_aL);
        lambda_aR = lambdac;
        lambda_aL = lambdac;
      }

      if (v_interface < lambda_aL)
        for (int v = 0; v < nf - 1; v++)
        {
          f[v] = fL[v] + lambdaL * (qaL[v] - qL[v]);
          q[v] = qaL[v];
        }
      else if (v_interface > lambda_aR)
        for (int v = 0; v < nf - 1; v++)
        {
          ...
          q[v] = qaR[v];
               ...
               (v_interface == lambda_aR))
        for (int v = 0; v < nf - 1; v++)
        {
          f[v] = (fR[v] + lambdaR * (qaR[v] - qR[v]) +
                  fL[v] + lambdaL * (qaL[v] - qL[v])) *
                 0.5;
          q[v] = 0.5 * (qaR[v] + qaL[v]);
        }
      ...
      else
      {
        ...
        else
        {
          ...
          {
            f[v] = (fR[v] + lambdaR * (qaR[v] - qR[v]) + lambda_aR * (qc[v] - qaR[v]) +
                    fL[v] + lambdaL * (qaL[v] - qL[v]) + lambda_aL * (qc2[v] - qaL[v])) *
                   0.5;
            q[v] = 0.5 * (qc[v] + qc2[v]);
          }
          free(qc2);
        ...
        free(qc);
        if ((vxaL >= lambdaL) & (vxaR <= lambdaR) & (vxcL >= lambda_aL) & (vxcR <= lambda_aR))
          consistency = 1;
        else
        {
          consistency = 0;
          if (PR)
            printf("hL, Pc = %+.6e %+.6e \n", hL, pc);
          if (PR)
            printf("vax_L, lambda_L, vcx_L, lambda_aL = %+.6e %+.6e %+.6e %+.6e \n", vxaL, lambdaL, vxcL, lambda_aL);
          if (PR)
            printf("vax_R, lambda_R, vcx_R, lambda_aR = %+.6e %+.6e %+.6e %+.6e \n", vxaR, lambdaR, vxcR, lambda_aR);
        }
      }
    ...
    if (consistency == 0)
    {
      if (convergence == 0)
        if (PR)
          ...
          printf("HLLD found a non-consistent state, switched to HLL \n");
      for (int v = 0; v < nf - 1; v++)
      {
        f[v] = fHLL[v];
        q[v] = qHLL[v];
