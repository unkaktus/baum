...
int matter_init(tL *level)
{
  ...
  if(MATTER.USENLS) {
    ...
    if(level->l > level->grid->lmin){
    	chiVarsc = give_chiVars_vl((level->grid->level[level->l-1]));
	enablevarlist(chiVarsc);
    }
  }
  ...
  MATTER.init(level);
  if(MATTER.USENLS){
         if(level->l > level->grid->lmin) mg_prolong(chiVarsc, chiVarsf, 1);//MATTER.chi_prolong_init(level, chiVarsc, chiVarsf);
	 MATTER.chi_init(level, chiVarsf);
...
void matter_rhs_prepare(tVarList *unew, tVarList *upre, double c, tVarList *ucur)
{
  ...
  // set ptrs to rhs
  for(v=0; v<MATTER.NVq; v++) {
    n = vldataptr(unew, MATTER.INDX_VAR_q + v);
    p = vldataptr(upre, MATTER.INDX_VAR_q + v);
...
{  
  ...
  // set ptrs to rhs
  for(v=0; v<MATTER.NVq; v++) {
    r = vldataptr(ucur, MATTER.INDX_VAR_q + v);
    forallpoints(level,i) 
      r[i] = 0.;
  }
}
...
void matter_rhs(tVarList *unew, tVarList *upre, double c, tVarList *ucur)
{
  ...
  tL *level = ucur->level;
  tL *lf = 0;
  if(level->l < level->grid->lmax) lf = level->grid->level[level->l+1];
  tVarList *matterADMVars = give_matterADMVars_vl(level);
  tVarList *primVars      = give_primVars_vl(level);
  tVarList *camrVars; 
  tVarList *nlsVars;
  ...
  if (MATTER.USENLS){
		  nlsVars       = give_nlsVars_vl(level);
		  chiVarsc	= give_chiVars_vl(level);
		  if(lf) chiVarsf = give_chiVars_vl(lf);
  ...
  if(MATTER.USENLS && timeforcompchi_dt(level, Getd("nls_comp_time"))){
    if(PR) printf("# ===> matter NLS chi\n");
    printf("Time to recompute chis\n");
    MATTER.chi_compute(level, chiVarsc);
    set_boundary_symmetry(level, chiVarsc);
    if(MATTER.COMP_CHI && lf) mg_prolong(chiVarsc, chiVarsf, 1);
  }

  if (PR) printf("# ===> matter rhs sources\n");
  timer_start(level, "matter_rhs_source");
  MATTER.sources(unew, upre, c, ucur, primVars, matterADMVars);
  ...
  matter_fluxes(unew, upre, c, ucur, primVars);
