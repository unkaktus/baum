...
int c_amr_mask(tL *level)
{
   ...
   if (MATTER.CAMRACTIVE == 0)
      return 0;

   /* For some levels we make nothing!*/
   if (level->l == 0)
      return 0;
   if (level->l < Geti("camr_level_min"))
      return 0;
   ...
   forallboxes(level)
   {
      ...
      xmminc = xminc - nbuffer * ldd;
      xmmaxc = xmaxc + nbuffer * ldd;
      ymminc = yminc - nbuffer * ldd;
      ymmaxc = ymaxc + nbuffer * ldd;
      zmminc = zminc - nbuffer * ldd;
      zmmaxc = zmaxc + nbuffer * ldd;

      if (PR)
         printf("xminc %e %e %e %e %e %e", xminc, xmaxc, yminc, ymaxc, zminc, zmaxc);
      ...
      forallpoints_boxijk(box)
      {
         ...
         if (dequal(xf[ijk], fbox[0]))
         {
            if (dlessgreater(fbox[2], yf[ijk], fbox[3]) &&
                dlessgreater(fbox[4], zf[ijk], fbox[5]))
               mask_B[ijk] = 1;
         }
         ...
         {
            if (dlessgreater(fbox[0], xf[ijk], fbox[1]) &&
                dlessgreater(fbox[2], yf[ijk], fbox[3]))
               mask_B[ijk] = 5;
         }
      ...
      forallpoints_ijk(g->level[lcoarse])
      {
         if (dequal(x[ijk], xminc))
         {
            if (dlessgreater(yminc, y[ijk], ymaxc) &&
                dlessgreater(zminc, z[ijk], zmaxc))
               mask_Ac[ijk] = 1;
         }
         ...
         {
            if (dlessgreater(xminc, x[ijk], xmaxc) &&
                dlessgreater(yminc, y[ijk], ymaxc))
               mask_Ac[ijk] = 5;
         }
         ...
         if (mask_Ac[ijk] == 0.7)
            if (!((dlessgreater(xmminc, x[ijk], xmmaxc)) &&
                  ...
                  (dlessgreater(zmminc, z[ijk], zmmaxc))))
               mask_Ac[ijk] = 0.;
      ...
      {
         mask_A[ijk] = 0.0;
         if (dequal(rflag[ijk], 2.0))
         { /* This if ensures that we stay in the correct box. */
            if (dequal(xc[ijk], xminc))
            {
               if (dlessgreater(yminc, yc[ijk], ymaxc) &&
                   dlessgreater(zminc, zc[ijk], zmaxc))
                  mask_A[ijk] = 1;
            }
            ...
            {
               if (dlessgreater(xminc, xc[ijk], xmaxc) &&
                   dlessgreater(yminc, yc[ijk], ymaxc))
                  mask_A[ijk] = 5;
            }
...
void correct_varlist(tG *g, int lfine, int lcoarse, tVarList *uf, tVarList *uc, int nbuffer)
{
   ...
   if (PR)
      printf("I am in correct_varlist!\n");
   ...
   if (lf->l == 0)
      return;
   if (!parentaligned(g->level[lfine]))
      return;
   if (dequal(lf->time, 0))
      return;
   ...
   if (PR)
      printf("I do not skip level \n");
   if (PR)
      printf("lmax %d lf %d lc %d \n", g->lmax, lf->l, lc->l);
         ...
         forallpoints_boxijk(box)
         {
            ...
            if (mask_A[ijk])
            {
              ...
              */
               ...
               {
                  if (PR)
                     printf("Do not correct for small densities.\n");
                  c[ijk] = 0;
               }
               ...
               if (DEBUG)
               {
                  ...
                  if (!dequal(mask_B[fijk], 0))
                     fc += 1;
                  if (!dequal(mask_B[fijk + fdi + fdj + fdk], 0))
                     fc += 1;
                  if (!dequal(mask_B[fijk + fdi + fdj], 0))
                     fc += 1;
                  if (!dequal(mask_B[fijk + fdi + fdk], 0))
                     fc += 1;
                  if (!dequal(mask_B[fijk + fdk + fdj], 0))
                     fc += 1;
                  if (!dequal(mask_B[fijk + fdi], 0))
                     fc += 1;
                  if (!dequal(mask_B[fijk + fdj], 0))
                     fc += 1;
                  if (!dequal(mask_B[fijk + fdk], 0))
                     fc += 1;
   ...
   if (PR)
      printf("I leave correct_varlist.\n");
...
int c_amr_time(tL *level)
{
   ...
   if (!(MATTER.CAMRACTIVE))
      printf("camr not switched on \n");
   else
      printf("camr ON \n");
