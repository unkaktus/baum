/* grhd_sources_Vac.c */
/* Copyright (C) 1998 Bernd Bruegmann, 20.7.2021 */
...
errorexit("alpha[ijk]<=ALPHAMIN"); 


} 


if(CheckForNANandINF(11,MD[ijk],MS1[ijk],MS2[ijk],MS3[ijk],Mtau[ijk],Mrho[ijk],Mepsl[ijk],Mp[ijk],Mv1[ijk],Mv2[ijk],Mv3[ijk])) { 
...
if(vsq>=1.0) W = GRHD.HRSC_WlorMAX; 
...
Tij11
=
W2hrho*pow2(vshift1) + Mp[ijk]*
   (ginv11 - pow2(beta1[ijk])*pow2inv(alpha[ijk]))
;

Tij12
=
vshift1*vshift2*W2hrho + Mp[ijk]*
   (ginv12 - beta1[ijk]*beta2[ijk]*pow2inv(alpha[ijk]))
;

Tij13
=
vshift1*vshift3*W2hrho + Mp[ijk]*
   (ginv13 - beta1[ijk]*beta3[ijk]*pow2inv(alpha[ijk]))
;

Tij22
=
W2hrho*pow2(vshift2) + Mp[ijk]*
   (ginv22 - pow2(beta2[ijk])*pow2inv(alpha[ijk]))
;

Tij23
=
vshift2*vshift3*W2hrho + Mp[ijk]*
   (ginv23 - beta2[ijk]*beta3[ijk]*pow2inv(alpha[ijk]))
;

Tij33
=
W2hrho*pow2(vshift3) + Mp[ijk]*
   (ginv33 - pow2(beta3[ijk])*pow2inv(alpha[ijk]))
;
...
beta1[ijk]*(-(conSi2/alpha[ijk]) + T00*beta2[ijk]) + 
  conSi1*(vup2 - beta2[ijk]/alpha[ijk]) + ginv12*Mp[ijk]
;

Tij13
=
beta1[ijk]*(-(conSi3/alpha[ijk]) + T00*beta3[ijk]) + 
  conSi1*(vup3 - beta3[ijk]/alpha[ijk]) + ginv13*Mp[ijk]
;
...
beta2[ijk]*(-(conSi3/alpha[ijk]) + T00*beta3[ijk]) + 
  conSi2*(vup3 - beta3[ijk]/alpha[ijk]) + ginv23*Mp[ijk]
;
...
/* grhd_sources_Vac.c */
/* nvars = 53, nauxs = 91, n* = 840,  n/ = 80,  n+ = 1198, n = 2118, O = 0 */
