...
tocompute = {
    ...
    Cinstruction == " if (((MATTER.USEMASK) && (mask[ijk]>.99)) || matrixdetg<=0.) {" ,
    ...
    Cinstruction == "if (vsq>=1.) W = GRHD.HRSC_WlorMAX;", 
        ...
        (* compute turbulent Tau *)
        Cinstruction == " if (alpha[ijk]>=0.2) {",

		(*3-velocity (up) partial derivatives *)
       		Cinstruction == "if (order_centered == 2 || boundaryNaway(1)) {",
	        	delv[a,b]    == del[a,Mv[b]],
       		Cinstruction == "} else if (order_centered == 4 || boundaryNaway(2)) {",
            		delv[a,b]    == del4[a,Mv[b]],
        	Cinstruction == "} else if (order_centered == 6 || boundaryNaway(3)) {",
            		delv[a,b]    == del6[a,Mv[b]],
        	Cinstruction == "} else errorexit(\"order is not implemented yet\");",

		Cinstruction	== "EOS.comp(\"re\",\"\",\"\",\"pc\",\"\",\"\", Mrho[ijk],Mepsl[ijk], &pres_eos,&cs2);\n",
		
		MturbTau[a,b]	== - lmix*sqrt[cs2]*W2hrho*(g[b,c]*delv[a,c] + g[a,c]*delv[b,c] + vup[c]*delg[c,a,b] - 1/3*g[a,b]*ginv[c,d]*(g[d,m]*delv[c,m] + 				g[c,m]*delv[d,m] + vup[m]*delg[m,c,d])),
        ...
        (* cure turb Tau *)
        Cinstruction == "if (CheckForNANandINF(6, MturbTau11[ijk], MturbTau12[ijk], MturbTau13[ijk], MturbTau22[ijk], MturbTau23[ijk], MturbTau33[ijk]) ) {",
            MturbTau[a,b] == 0,
        Cinstruction == "}",
        ...
        Cif == useprim,
            ...
            Tij[a,b] == W2hrho*vshift[a]*vshift[b] + Mp*(ginv[a,b]-beta[a]*beta[b]/(alpha*alpha)) + MturbTau[a,b],
...
MturbTau[a_,b_] := MturbTau[b,a] /; !OrderedQ[{a,b}]
...
functionname = "grhd_sources_turb";
...
variabledeclarations[] := Module[{},
  ...
  prdecvlNR[{MturbTau[a,b]}, "otherVars", "GRHD.IND_turbTau"];
